...
Situation: we have bivariate ($X,Y$) data and assume there is a positive proportional relationship between $X$ and $Y$. 
\vfill
...
1. To estimate the ratio of two population characteristics. 
\vfill
\vfill

2. To use the relationship 
\vfill
\vfill

Note that ratio estimation 
\vfill
...
Example: Suppose we can predict MSU statistics students future income, using the average GPA in statistics courses.
	...
	Compute $\hat{B}$ 
	\vfill
	It is known that the average GPA for all statistics students is 3.00. Compute $\hat{\bar{y}}_r$ \vfill
	
\newpage	

Case 2: $t_x$ and $\bar{x}_U$ are unknown

If $t_x$ and $\bar{x}_U$ are unknown, it will not affect the estimator $\hat{B} = \bar{y} / \bar{x}$. 
\vfill

In such cases, it is common to replace $t_x$ with $N \bar{x}$ or replace $\bar{x}_U$ with $\bar{x}$. then 
\vfill
...
However, when used appropriately the 
\vfill
...
In this case, $t_y =$ `r round(t.y)`. Based on a single sample, the ratio estimator is off by
